...
class CGPSTest {
    ...
    fun actualLocation() {
        runBlocking {
            try {
                val location = HardwareCGPS(ApplicationProvider.getApplicationContext()).actualLocation()
    ...
    fun lastLocation() {
        runBlocking {
            try {
                val location = HardwareCGPS(ApplicationProvider.getApplicationContext()).lastLocation()
    ...
    fun requestUpdates() {
        ...
        runBlocking {
            job = launch {
                HardwareCGPS(ApplicationProvider.getApplicationContext()).requestUpdates().collect {
                    locationList.add(it)
